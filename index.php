...
/* VOIR DOCUMENTATION DANS "output/index.html" */

/*
Pour générer une documentation PHP via Composer :
https://www.youtube.com/watch?v=675O2oDmr2w
Dans le terminal :
Dans "monsite":
php composer.phar init
php composer.phar install ou php composer.phar update
vendor/bin/phpdoc
exemple : vendor/bin/phpdoc --directory="inc"
puis ouvrir "index.html" dans le nouveau dossier "output"
*/
...
switch($sPage)
{
	...
	case 'thanks_order':
	if(empty($_SESSION['id'])) // Utilisateur non connecté
	{
		echo "<script type='text/javascript'>document.location.replace('user_basket');</script>";
	}
	else
	{
		$Id_Order = recup_last_order_id_user($_SESSION['id']);
		$Id_Order_Max = recup_last_order_id();

		if(is_null($Id_Order['id']) || ($Id_Order_Max['id'] != $Id_Order['id']))
		{
			echo "<script type='text/javascript'>document.location.replace('user_basket');</script>";
		}
		else
	...
	if(empty($_SESSION['id'])) // Utilisateur non connecté
	{
		echo "<script type='text/javascript'>document.location.replace('user_order');</script>";
	}
	else // Utilisateur connecté
	{
		if($_SESSION['basket']['total'] == 0) // Panier Vide
		{
			echo "<script type='text/javascript'>document.location.replace('user_basket');</script>";
		}
		else // Panier rempli
		{
			if(!VerifContactUser($_SESSION['pseudo'])) // Coordonnées de l'utilisateur non remplies
			{
				echo "<script type='text/javascript'>document.location.replace('user_order_contact_information');</script>";
			}
			else // Coordonnées de l'utilisateur remplies
			{
				include('views/confirm_order.phtml');
				if(isset($_POST) && isset($_POST['order_confirm']) && isset($_POST['cgv_yes']) && $_POST['cgv_yes'] == "on" && ($_SESSION['basket']['total'] != 0) && !empty($_SESSION['id']))
				{
	...
	case 'user_check_contact_information':
		...
		case 'user_order_contact_information':
			...
			case 'user_order':
			if(isset($_SESSION['id']) && isset($_SESSION['pseudo'])) // Si l'utilisateur est déjà connecté
			{
				if(VerifContactUser($_SESSION['pseudo']))
				{
					echo "<script type='text/javascript'>document.location.replace('user_check_contact_information');</script>";
				}
				else
			...
			else
			{
				...
				if(isset($_POST) && isset($_POST['check_user']))
				{
					...
					if (is_string($_POST['pseudo']) && is_string($_POST['password']) && VerifExistUser($_POST['pseudo'], $_POST['password']))
					{
					...
					if(isset($_POST) && isset($_POST['envoyer']) && isset($_SESSION['id']))
					{
						...
						if(!empty($_FILES['picture']['name']))
						{
							...
							*/
							...
							if(getimagesize($_FILES['picture']['tmp_name']) == false) // Retourne la taille d'une image
							{
								$bError = true;
								$sError_Message = "Le fichier n'a pas le bon format";
							}
						...
						if(isset($_POST) && isset($_POST['modify_user_profile']))
						{
							if(($_POST['user_id'] == $_SESSION['id']) && ($_POST['pseudo_user_id'] == $_SESSION['pseudo']))
							{
								...
								if(!empty($_FILES['avatar_user_profile']['name']))
								{
									...
									if(getimagesize($_FILES['avatar_user_profile']['tmp_name']) == false) // Retourne la taille d'une image
									{
										$bError = true;
										$sError_Message = "Le fichier n'a pas le bon format";
									}
						...
						if(isset($_POST) && isset($_POST['submit_contact']))
						{
							if(
								...
								)
								{
									...
									if(!empty($_POST['prestation_contact']) && is_string($_POST['prestation_contact']) && array_key_exists($_POST['prestation_contact'], $aPrestations))
									...
									{
										echo "<p class='warning_connect'><i class='fa fa-exclamation-triangle' aria-hidden='true'></i> ERREUR ==> Prestation incorrecte !</p>";
										break;
									}

									if(!empty($_POST['date_contact']) && is_string($_POST['date_contact']))
									{
										$aTab_Date = explode("-", $_POST['date_contact']);

										$Date_Courante = new DateTime(date("Y-m-d"));
										$Date_Post = new DateTime($_POST['date_contact']);

										if(checkdate($aTab_Date[1],$aTab_Date[2],$aTab_Date[0]) && ($Date_Courante <= $Date_Post)) // checkdate : Valide une date grégorienne (M,J,A)
										{

											// VERIFICATION DE L'ANNEE
											if($aTab_Date[0] >= 2017 && $aTab_Date[0] < 2100)
											{
												$iYear = $aTab_Date[0];
											}
											else
											{
												echo "<p class='warning_connect'><i class='fa fa-exclamation-triangle' aria-hidden='true'></i> ERREUR ==> Année incorrecte !</p>";
												break;
											}
											...
											if($sMonth == "")
											{
												echo "<p class='warning_connect'><i class='fa fa-exclamation-triangle' aria-hidden='true'></i> ERREUR ==> Mois incorrecte !</p>";
												break;
											}

											// VERIFICATION DU JOUR
											if($aTab_Date[2] >= 01 && $aTab_Date[2] <= 31)
											{
												$iDay = $aTab_Date[2];
											}
											else
											{
												echo "<p class='warning_connect'><i class='fa fa-exclamation-triangle' aria-hidden='true'></i> ERREUR ==> Jour incorrecte !</p>";
												exit;
											}
										}
										else
										{
											echo "<p class='warning_connect'><i class='fa fa-exclamation-triangle' aria-hidden='true'></i> ERREUR ==> Date incorrecte !</p>";
											exit;
										}
									}
									else
									...
									if(!empty($_POST['hour_begin_contact']) && is_string($_POST['hour_begin_contact']) && array_key_exists($_POST['hour_begin_contact'], $aHours))
									{
										$iHour_Begin = $_POST['hour_begin_contact'];
									}
									else
									{
										echo "<p class='warning_connect'><i class='fa fa-exclamation-triangle' aria-hidden='true'></i> ERREUR ==> Heure de début incorrecte !</p>";
										break;
									}
									// VERIFICATION DES MINUTES DE DEBUT
									if(!empty($_POST['minutes_begin_contact']) && is_string($_POST['minutes_begin_contact']) && array_key_exists($_POST['minutes_begin_contact'], $aMinutes))
									{
										$iMinutes_Begin = $_POST['minutes_begin_contact'];
									}
									else
									{
										echo "<p class='warning_connect'><i class='fa fa-exclamation-triangle' aria-hidden='true'></i> ERREUR ==> Minutes de début incorrecte !</p>";
										break;
									}

									// VERIFICATION DE L'HEURE DE FIN
									if(!empty($_POST['hour_end_contact']) && is_string($_POST['hour_end_contact']) && array_key_exists($_POST['hour_end_contact'], $aHours))
									{
										$iHour_End = $_POST['hour_end_contact'];
									}
									else
									{
										echo "<p class='warning_connect'><i class='fa fa-exclamation-triangle' aria-hidden='true'></i> ERREUR ==> Heure de fin incorrecte !</p>";
										break;
									}
									// VERIFICATION DES MINUTES DE FIN
									if(!empty($_POST['minutes_end_contact']) && is_string($_POST['minutes_end_contact']) && array_key_exists($_POST['minutes_end_contact'], $aMinutes))
									{
										$iMinutes_End = $_POST['minutes_end_contact'];
									}
									else
									{
										echo "<p class='warning_connect'><i class='fa fa-exclamation-triangle' aria-hidden='true'></i> ERREUR ==> Minutes de fin incorrecte !</p>";
										break;
									}

									// VERIFICATION DU NOMBRE DE PARTICIPANTS
									if(!empty($_POST['persons_number']) && is_string($_POST['persons_number']) && array_key_exists($_POST['persons_number'], $aNumber_People))
									{
										$iNumber_People = $_POST['persons_number'];
									}
									else
									{
										echo "<p class='warning_connect'><i class='fa fa-exclamation-triangle' aria-hidden='true'></i> ERREUR ==> Nombre de participants incorrecte !</p>";
										break;
									}
									...
									{
										$_POST['message_contact'] = htmlspecialchars($_POST['message_contact']);
									}
									...
									" Heure de Debut : ".$iHour_Begin.":".$iMinutes_Begin.$passage_ligne.
									" Heure de Fin : ".$iHour_End.":".$iMinutes_End.$passage_ligne.
									" Nombre de Personnes : ".$iNumber_People.$passage_ligne.
									...
									"<p><b> Heure de Début : </b>".$iHour_Begin.":".$iMinutes_Begin."</p>".$passage_ligne.
									"<p><b> Heure de Fin : </b>".$iHour_End.":".$iMinutes_End."</p>".$passage_ligne.
									"<p><b> Nombre de Personnes : </b>".$iNumber_People."</p>".$passage_ligne.
